...
float Pedals::getThrottle()
{
    ...
    float apps1_voltage = IO::Get()->adc2_handle->readVoltage(0);
    float apps2_voltage = IO::Get()->adc2_handle->readVoltage(5);
    // float apps1_voltage = Sensors::Get()->get_sensor_voltages()[Sensors::APPS1];
    // float apps2_voltage = Sensors::Get()->get_sensor_voltages()[Sensors::APPS2];
